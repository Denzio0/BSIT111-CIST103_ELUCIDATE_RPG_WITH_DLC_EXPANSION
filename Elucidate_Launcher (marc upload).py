...
def draw_verify_bg():
	...
	pygame.draw.rect(screen, (18, 18, 18), (0, 0, screen_x, screen_y))
	pygame.draw.rect(screen, (70, 70, 65), (0, 0, screen_x, screen_y), 1)
	pygame.draw.rect(screen, (25, 25, 25), (0, screen_y - 40, screen_x, 40))
	draw_text_ui("ELUCIDATE  ·  ASSET VERIFICATION SYSTEM", screen_x // 2 - 160, screen_y - 28, (90, 88, 84))

def handle_quit():
	global screen_x, screen_y, screen
...
def _draw_zip_panel(status_line, file_line, pct, phase_label):
	...
	pygame.draw.rect(screen, (38, 38, 38), (_pad, _y, _bw, 44))
	pygame.draw.rect(screen, (80, 80, 75), (_pad, _y, _bw, 44), 1)
	_ts = font.render("ELUCIDATE RPG  —  PACKAGE INSTALLATION SYSTEM", True, (210, 205, 195))
	screen.blit(_ts, (_mid - _ts.get_width() // 2, _y + 13))
	_y += 44 + _gap

	pygame.draw.rect(screen, (28, 28, 28), (_pad, _y, _bw, 32))
	pygame.draw.rect(screen, (60, 60, 58), (_pad, _y, _bw, 32), 1)
	draw_text_ui(phase_label, _pad + 14, _y + 8, (170, 165, 155))
	_y += 32 + _gap

	_ob_top = _y
	pygame.draw.rect(screen, (22, 22, 22), (_pad, _ob_top, _bw, 90))
	pygame.draw.rect(screen, (65, 65, 62), (_pad, _ob_top, _bw, 90), 1)
	_y = _ob_top + 8
	draw_text_ui("EXTRACTION PROGRESS", _pad + 14, _y, (140, 135, 125))
	_y += 20
	pygame.draw.rect(screen, (30, 30, 30), (_pad + 10, _y, _bw - 20, 22))
	pygame.draw.rect(screen, (60, 60, 58), (_pad + 10, _y, _bw - 20, 22), 1)
	_ofw = int(min(pct, 100) / 100 * (_bw - 20))
	if _ofw > 0:
		pygame.draw.rect(screen, (180, 180, 160), (_pad + 10, _y, _ofw, 22))
		pygame.draw.rect(screen, (230, 230, 210), (_pad + 10, _y, _ofw, 4))
	_y += 22 + 6
	draw_text_ui(f"EXTRACTED  {int(pct)}%", _pad + 14, _y, (200, 195, 185))
	_y = _ob_top + 90 + _gap

	_cf_top = _y
	pygame.draw.rect(screen, (22, 22, 22), (_pad, _cf_top, _bw, 56))
	pygame.draw.rect(screen, (55, 55, 52), (_pad, _cf_top, _bw, 56), 1)
	_y = _cf_top + 8
	draw_text_ui("CURRENT ENTRY", _pad + 14, _y, (120, 115, 105))
	_y += 20
	draw_text_ui("FILE   ›   " + (file_line[:90] if file_line else "—"), _pad + 14, _y, (220, 215, 205))
	_y = _cf_top + 56 + _gap

	pygame.draw.rect(screen, (22, 22, 22), (_pad, _y, _bw, 40))
	pygame.draw.rect(screen, (55, 55, 52), (_pad, _y, _bw, 40), 1)
	draw_text_ui("STATUS   " + status_line, _pad + 14, _y + 11, (190, 185, 175))
...
def zip_download(url):
	...
	_draw_zip_panel(
		status_line="⧖  SCANNING PACKAGE INTEGRITY...",
		file_line="",
		pct=0,
		phase_label="PHASE   1  /  3   —   PRE-FLIGHT VALIDATION"
	)
	...
	try:
		...
		_draw_zip_panel(
			status_line="↓  CONNECTED TO PACKAGE SERVER — DOWNLOADING REQUIRED FILES",
			file_line="package bundle",
			pct=0,
			phase_label="PHASE   2  /  3   —   DOWNLOADING COMPRESSED PACKAGE"
		)
		gdown.download(url, temp_zip, quiet=False)

		if not os.path.exists(temp_zip) or os.path.getsize(temp_zip) < 10000:
			_draw_zip_panel(
				status_line="✖  PACKAGE INVALID OR CORRUPTED — ABORTING",
				file_line="",
				pct=0,
				phase_label="PHASE   —   VALIDATION FAILED"
			)
			pygame.time.delay(2000)
			return False

		_draw_zip_panel(
			status_line="⧗  CONNECTED TO PACKAGE — USING DEFLATE COMPRESSION TO EXTRACT ",
			file_line="reading archive...",
			pct=0,
			phase_label="PHASE   3  /  3   —   PACKAGE EXTRACTION"
		)
		...
		with zipfile.ZipFile(temp_zip, 'r') as zip_ref:
			...
			for idx, member in enumerate(all_files):
				...
				_draw_zip_panel(
					status_line="⧗  CONNECTED TO PACKAGE — EXTRACTING REQUIRED FILES",
					file_line=member,
					pct=pct,
					phase_label="PHASE   3  /  3   —   PACKAGE EXTRACTION"
				)
		...
		for pidx, item in enumerate(patch_items):
			...
			_draw_zip_panel(
				status_line="⇄  APPLYING PATCH FILES TO INSTALLATION DIRECTORY",
				file_line=item,
				pct=pct,
				phase_label="PHASE   3  /  3   —   WRITING PATCH FILES"
			)
		...
		_draw_zip_panel(
			status_line="✔  PACKAGE INSTALLATION COMPLETE",
			file_line="all files installed successfully",
			pct=100,
			phase_label="PHASE   —   INSTALLATION COMPLETE"
		)
		...
		_draw_zip_panel(
			status_line="✖  CRITICAL ERROR — INSTALLATION ABORTED",
			file_line=str(e)[:90],
			pct=0,
			phase_label="PHASE   —   ERROR"
		)
...
def verify(filename, url):
	...
	if os.path.exists(filename):
		_g_current_status = "✔  ALREADY ON DEVICE — VERIFIED"
		_g_total_bytes    = 0
		_g_received_bytes = 0
		_g_last_pct       = 100
		handle_quit()
		...
		_g_current_status = "✖  NOT FOUND — CONNECTING TO SERVER..."
		_g_total_bytes    = 0
		_g_received_bytes = 0
		_g_last_pct       = 0
		handle_quit()
		...
		for attempt in range(30):
			...
			try:
				with requests.get(url, headers=headers, stream=True, timeout=10) as r:
					...
					_g_received_bytes = 0
					_g_current_status = "↓  CONNECTED — TRANSFERRING DATA"
					with open(filename, "wb") as f:
						for chunk in r.iter_content(1024):
							if chunk:
								...
								handle_quit()
								_draw_global_panel()
				_g_current_status = "✔  TRANSFER COMPLETE"
				_g_last_pct = 100
				_draw_global_panel()
				break
			except Exception:
				tries += 1
				_g_current_status = f"↺  INTERRUPTED — RETRYING  ({tries}/30)"
				_draw_global_panel()
				pygame.time.delay(100)
		else:
			_g_current_status = "✖  DOWNLOAD FAILED — ASSET UNAVAILABLE"
			_draw_global_panel()
...
def _draw_global_panel():
	...
	pygame.draw.rect(screen, (38, 38, 38), (_pad, _y, _bw, 44))
	pygame.draw.rect(screen, (80, 80, 75), (_pad, _y, _bw, 44), 1)
	_ts = font.render("ELUCIDATE RPG  —  ASSET VERIFICATION & DOWNLOAD", True, (210, 205, 195))
	screen.blit(_ts, (_mid - _ts.get_width() // 2, _y + 13))
	_y += 44 + _gap

	pygame.draw.rect(screen, (28, 28, 28), (_pad, _y, _bw, 32))
	pygame.draw.rect(screen, (60, 60, 58), (_pad, _y, _bw, 32), 1)
	draw_text_ui(f"ASSET   {_g_done_assets}  /  {_g_total_assets}", _pad + 14, _y + 8, (170, 165, 155))
	_y += 32 + _gap
	...
	pygame.draw.rect(screen, (65, 65, 62), (_pad, _ob_top, _bw, 90), 1)
	_y = _ob_top + 8
	draw_text_ui("OVERALL PROGRESS", _pad + 14, _y, (140, 135, 125))
	_y += 20
	pygame.draw.rect(screen, (15, 15, 15), (_pad + 10, _y, _bw - 20, 22))
	pygame.draw.rect(screen, (30, 30, 30), (_pad + 10, _y, _bw - 20, 22))
	pygame.draw.rect(screen, (60, 60, 58), (_pad + 10, _y, _bw - 20, 22), 1)
	...
	if _ofw > 0:
		...
		pygame.draw.rect(screen, (230, 230, 210), (_pad + 10, _y, _ofw, 4))
	_y += 22 + 6
	draw_text_ui(f"OVERALL  {_op}%", _pad + 14, _y, (200, 195, 185))
	_y = _ob_top + 90 + _gap

	_cf_top = _y
	pygame.draw.rect(screen, (22, 22, 22), (_pad, _cf_top, _bw, 80))
	pygame.draw.rect(screen, (55, 55, 52), (_pad, _cf_top, _bw, 80), 1)
	_y = _cf_top + 8
	draw_text_ui("CURRENT FILE", _pad + 14, _y, (120, 115, 105))
	_y += 20
	draw_text_ui("FOLDER   ›   " + (_g_current_folder if _g_current_folder else "root"), _pad + 14, _y, (140, 135, 128))
	_y += 22
	draw_text_ui("FILE     ›   " + _g_current_file, _pad + 14, _y, (220, 215, 205))
	_y = _cf_top + 80 + _gap

	_fb_top = _y
	pygame.draw.rect(screen, (22, 22, 22), (_pad, _fb_top, _bw, 90))
	pygame.draw.rect(screen, (55, 55, 52), (_pad, _fb_top, _bw, 90), 1)
	_y = _fb_top + 8
	draw_text_ui("FILE PROGRESS", _pad + 14, _y, (140, 135, 125))
	_y += 20
	pygame.draw.rect(screen, (15, 15, 15), (_pad + 10, _y, _bw - 20, 22))
	pygame.draw.rect(screen, (30, 30, 30), (_pad + 10, _y, _bw - 20, 22))
	pygame.draw.rect(screen, (55, 55, 52), (_pad + 10, _y, _bw - 20, 22), 1)
	if _g_total_bytes > 0:
	...
	if _ffw > 0:
		pygame.draw.rect(screen, (160, 160, 145), (_pad + 10, _y, _ffw, 22))
		pygame.draw.rect(screen, (210, 210, 190), (_pad + 10, _y, _ffw, 3))
	_y += 22 + 6
	draw_text_ui(f"FILE  {_fp}%", _pad + 14, _y, (190, 185, 175))
	_y = _fb_top + 90 + _gap

	pygame.draw.rect(screen, (22, 22, 22), (_pad, _y, _bw, 56))
	pygame.draw.rect(screen, (55, 55, 52), (_pad, _y, _bw, 56), 1)
	draw_text_ui("RECEIVED   " + _size_str,         _pad + 14, _y + 10, (160, 155, 145))
	draw_text_ui("STATUS     " + _g_current_status, _pad + 14, _y + 32, (190, 185, 175))
	_y += 56 + _gap
	...
	pygame.draw.rect(screen, (22, 22, 22), (_pad, _y, _bw, 34))
	pygame.draw.rect(screen, (55, 55, 52), (_pad, _y, _bw, 34), 1)
	draw_text_ui("TIME TAKEN   " + _time_str, _pad + 14, _y + 8, (155, 150, 140))
...
for i in range(1):
	...
	verify("sprites/npc_e_luminarian_spy/elucidate_walk_church_spy_npc_right_001", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/elucidate_walk_church_spy_npc_right_001.png"); _g_done_assets += 1
	...
	verify("wiki_images/wiki_elucidate_characterspage.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_characterspage.png")
	verify("wiki_images/wiki_elucidate_developerspage.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_developerspage.png")
	verify("wiki_images/wiki_elucidate_downloadpage.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_downloadpage.png")
	verify("wiki_images/wiki_elucidate_environmentspage1.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_environmentspage1.png")
	verify("wiki_images/wiki_elucidate_environmentspage2.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_environmentspage2.png")
	verify("wiki_images/wiki_elucidate_fanartspage.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_fanartspage.png")
	verify("wiki_images/wiki_elucidate_homepage.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_homepage.png")
	verify("wiki_images/wiki_elucidate_itemspage1.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_itemspage1.png")
	verify("wiki_images/wiki_elucidate_itemspage2.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_itemspage2.png")
	verify("wiki_images/wiki_elucidate_lorepage.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_lorepage.png")
	verify("wiki_images/wiki_elucidate_npcspage1.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_npcspage1.png	")
	verify("wiki_images/wiki_elucidate_npcspage2.png", "http://elucidatewebstorage.unaux.com/wp-content/uploads/2026/04/wiki_elucidate_npcspage2.png")
...
while True:
	...
	if jhvb.collidepoint(mouse_x, mouse_y):
		...
		for event in events:
			if event.type == pygame.MOUSEBUTTONDOWN:
				screen = pygame.display.set_mode((1275, 710))
				loop = True
				...
				while loop:
					...
					if log1.collidepoint(mouse_x, mouse_y):
						...
						for event in events:
							if event.type == pygame.MOUSEBUTTONDOWN:
								zip_download("https://drive.google.com/uc?id=1KrinhBtAZE0bC5PU44srZCKB3Uqyw5xn")
								loop = False
					if log2.collidepoint(mouse_x, mouse_y):
						bg = 2
						for event in events:
							if event.type == pygame.MOUSEBUTTONDOWN:
								zip_download("https://drive.google.com/uc?id=17ASP6oEGCks-A-og4bez9cc8m2m9zKc8")
								loop = False
